...
const getDailyPeriodCode = (periodName, teacherId) => {
  ...
  const dateStr = `${today.getFullYear()}-${
    today.getMonth() + 1
  }-${today.getDate()}-${teacherId}-${periodName}`;
  let hash = 0;
...
export default function App() {
  ...
  if (loading) {
    return (
      ...
      >
        <h2 style={{ color: '#1e293b', fontSize: '24px', marginBottom: '8px' }}>
          Loading SmartPass...
        </h2>
        ...
        >
          ...
          Please click the <b>"Open in New Window"</b> button in the top right
          of StackBlitz!
        </p>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans flex flex-col">
      ...
      <main className="max-w-6xl mx-auto w-full p-6 flex-1">
        ...
        {view === 'teacher-auth' && (
          <TeacherAuthView auth={auth} setView={setView} />
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-2xl">
        ...
        >
          ...
          <p className="text-sm text-slate-500 mt-2 text-center">
            Request a hall pass by entering your student ID.
          </p>
        </button>
        ...
        >
          ...
          <p className="text-sm text-slate-500 mt-2 text-center">
            Log in to manage your classes, rosters, and passes.
          </p>
        </button>
      </div>
    </div>
    ...
    <div className="max-w-md mx-auto mt-12 bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
      <div className="text-center mb-6">
        ...
        <h2 className="text-2xl font-bold text-slate-800">
          {isRegister ? 'Teacher Registration' : 'Teacher Login'}
        </h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {error && (
          <p className="text-red-500 text-xs font-semibold bg-red-50 p-3 rounded-xl">
            {error}
          </p>
        )}
      ...
      <div className="max-w-md mx-auto mt-12 bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden text-center">
        ...
        >
          <h2 className="text-3xl font-bold mb-2">
            {isWaiting ? 'Waiting for Approval' : 'Pass Approved!'}
          </h2>
          <p className="text-white/80 font-medium text-lg">
            {isWaiting
              ? 'Keep this screen open.'
              : 'You may go to your destination.'}
          </p>
        </div>
        <div className="p-8 space-y-4">
          <p className="text-2xl font-bold text-slate-800">{student.name}</p>
          <p className="text-slate-600 text-lg">
            {DESTINATIONS[activePass.destination]?.icon}{' '}
            {DESTINATIONS[activePass.destination]?.label}
          </p>
        </div>
      </div>
    );
      ...
      {!selectedTeacherId ? (
        <div className="space-y-6">
          <div className="text-center">
            ...
            <p className="text-slate-500 text-sm mt-1">
              Choose your teacher to request a pass.
            </p>
          </div>
          <div className="space-y-2">
            {teachers.length === 0 ? (
              <p className="text-center text-slate-400 py-6 text-sm">
                No teachers have registered yet. Ask your teacher to set up
                their account.
              </p>
            ) : (
              teachers.map((t) => (
        ...
        <form onSubmit={handleStudentIdSubmit} className="space-y-6">
          ...
          <div className="flex space-x-3">
            ...
            <button
              type="submit"
              className="w-2/3 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-sm shadow-sm"
            >
              Next
            </button>
          </div>
        ...
        <form onSubmit={handleCodeSubmit} className="space-y-6">
          <div className="text-center">
            ...
            <p className="text-slate-500 text-sm mt-1">
              Enter today's 4-digit class code for {student.period}.
            </p>
          </div>
          <div>
            <input
          ...
          <button
            ...
            className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 text-white font-bold rounded-xl shadow-sm"
          >
            Unlock Pass
          </button>
        </form>
          ...
          <div className="grid grid-cols-2 gap-3">
            {Object.entries(DESTINATIONS).map(([key, dest]) => (
              <button
                ...
                className={`p-3 border rounded-xl flex items-center space-x-2 text-left font-semibold text-sm transition-all ${
                  destination === key
...
function TeacherDashboardView({ db, appId, user, setView }) {
  ...
  const addStudentToRoster = async (e) => {
    ...
    setRosterError('');
    await setDoc(
      doc(db, 'artifacts', appId, 'users', teacherId, 'roster', cleanId),
      {
        studentId: cleanId,
  ...
  const deleteStudentFromRoster = async (studentId) => {
    await deleteDoc(
      doc(db, 'artifacts', appId, 'users', teacherId, 'roster', studentId)
    );
  };
  ...
  const handleBulkImport = async (e) => {
    ...
    setBulkMessage(
      `Successfully imported ${addedCount} student(s). Skipped ${skippedCount} duplicate/invalid entries.`
    );
    setBulkInput('');
  };
  ...
  const processFile = (file) => {
    ...
    reader.onload = async (event) => {
      ...
      setBulkInput(text);
      setBulkMessage(
        `Loaded file "${file.name}". Click "Import List" below to finish saving.`
      );
    };
  ...
  const handlePrintHistoricalReport = async () => {
    ...
    const dateStr = `${dateObj.getFullYear()}-${
      dateObj.getMonth() + 1
    }-${dateObj.getDate()}-${teacherId}-${historicalClassName}`;
    let hash = 0;
    for (let i = 0; i < dateStr.length; i++) {
      hash = (hash << 5) - hash + dateStr.charCodeAt(i);
      hash |= 0;
    ...
    try {
      const passesRef = collection(
        ...
        teacherId,
        'sessions',
        historicalSessionCode,
        'passes'
      );
      const snapshot = await getDocs(passesRef);
      const passesList = snapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
  ...
  const waitingPasses = passes
    .filter((p) => p.status === 'waiting')
    .sort((a, b) => b.timestamp - a.timestamp);
  const activePasses = passes
    .filter((p) => p.status === 'approved')
    .sort((a, b) => b.timestamp - a.timestamp);
  const returnedPasses = passes
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between rounded-2xl shadow-sm">
          ...
          <div>
            <div className="flex items-center">
              ...
              <span
                ...
                title="Students will see this name"
              >
                ✏️
              </span>
            </div>
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            className="text-indigo-600 bg-indigo-50 hover:bg-indigo-100 px-3 py-1.5 rounded-lg text-sm font-bold flex items-center shadow-sm"
          >
            <Info size={16} className="mr-1.5" /> Help & About
          </button>
          <button
            onClick={() => window.print()}
            className="hidden sm:flex text-slate-600 bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-lg text-sm font-bold items-center shadow-sm"
          >
          ...
          <button
            ...
            className="text-red-600 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg text-sm font-bold flex items-center"
          >
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-6 print:hidden">
          <div>
            <h2 className="text-xl font-bold text-slate-800">
              Manage Student Roster & ID Numbers
            </h2>
            <p className="text-slate-500 text-sm mt-1">
              Add student IDs individually, paste lists, or drag and drop a CSV
              file below.
            </p>
          </div>
          <form
            onSubmit={addStudentToRoster}
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
              ...
              <div className="flex items-end">
                <button
                  type="submit"
                  className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-sm shadow-sm"
                >
                  Add Student
                </button>
              </div>
          ...
          >
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center">
                <Upload size={14} className="mr-1.5 text-indigo-600" /> Bulk
                Import / Drag & Drop CSV
              </h3>
              <div className="flex items-center space-x-3 w-full sm:w-auto">
                <span className="text-xs text-slate-500">
                  Default period if CSV lacks it:
                </span>
                <select
                  value={bulkPeriodInput}
                  onChange={(e) => setBulkPeriodInput(e.target.value)}
                  className="p-1.5 border border-slate-300 rounded-lg text-xs outline-none bg-white font-bold"
                >
                  {['Period 1', 'Period 2', 'Period 3', 'Period 4'].map((p) => (
                    <option key={p} value={p}>
                      {p}
            ...
            >
              <div className="flex flex-col items-center justify-center space-y-2">
                ...
                <p className="text-[11px] text-slate-400">
                  Supports .csv files (Format: Student ID, Student Name,
                  [Period])
                </p>
              </div>
            </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <button
                ...
                className="px-6 py-2 bg-slate-800 hover:bg-slate-900 disabled:bg-slate-300 text-white font-bold rounded-xl text-xs transition-colors shadow-sm"
              >
                Import List
              </button>
            </div>
              ...
              {roster.map((student) => (
                <tr key={student.studentId} className="hover:bg-slate-50">
                  ...
                  <td className="py-3 px-4 text-right">
                    <button
                      onClick={() => deleteStudentFromRoster(student.studentId)}
            ...
            <div className="bg-indigo-600 text-white rounded-2xl p-6 flex flex-col justify-between shadow-sm">
              <div>
                <h2 className="text-lg font-bold flex items-center">
                  <Key className="mr-2" size={18} /> Daily Session Code
                </h2>
                  ...
                  {['Period 1', 'Period 2', 'Period 3', 'Period 4'].map((p) => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative py-3 px-3 rounded-xl text-xs font-bold transition-all ${
                          className === p
                      ...
                      >
                        {p}
                        {count > 0 && (
                          <span className="absolute -top-1.5 -right-1.5 bg-red-500 text-white text-[10px] w-5 h-5 rounded-full flex items-center justify-center font-extrabold shadow-sm">
                            {count}
                          </span>
                        )}
                      </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-[60vh]">
              <div className="bg-amber-500 p-4 flex justify-between items-center text-white shrink-0">
                <h2 className="text-lg font-bold flex items-center">
                  <Clock size={18} className="mr-2" /> Pending Requests
                </h2>
                <span className="bg-amber-600 px-2.5 py-0.5 rounded-full text-xs font-bold">
                  {waitingPasses.length}
                </span>
              </div>
                  ...
                  >
                    <div>
                      ...
                      <p className="text-slate-600 text-xs mt-0.5">
                        {DESTINATIONS[pass.destination]?.icon}{' '}
                        {DESTINATIONS[pass.destination]?.label}
                      </p>
                    </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-[60vh]">
              <div className="bg-emerald-500 p-4 flex justify-between items-center text-white shrink-0">
                <h2 className="text-lg font-bold flex items-center">
                  <ArrowRight size={18} className="mr-2" /> Currently Out
                </h2>
                <span className="bg-emerald-600 px-2.5 py-0.5 rounded-full text-xs font-bold">
                  {activePasses.length}
                </span>
              </div>
              <div className="p-4 flex-1 overflow-y-auto bg-slate-50/50 space-y-3">
                {activePasses.map((pass) => (
                  <div
                    key={pass.id}
                    className="bg-white p-4 rounded-xl border-l-4 border-l-emerald-500 border border-emerald-100 shadow-sm flex items-center justify-between"
                  >
                    <div>
                      <h3 className="font-bold text-slate-800">
                        {pass.studentName}
                    ...
                    <button
                      onClick={() => updatePassStatus(pass.id, 'returned')}
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-[60vh]">
              <div className="bg-slate-600 p-4 flex justify-between items-center text-white shrink-0">
                <h2 className="text-lg font-bold flex items-center">
                  <Calendar size={18} className="mr-2" /> Session History
                </h2>
                <span className="bg-slate-700 px-2.5 py-0.5 rounded-full text-xs font-bold">
                  {returnedPasses.length}
                </span>
              </div>
              <div className="p-4 flex-1 overflow-y-auto bg-slate-50/50 space-y-3">
                {returnedPasses.map((pass) => (
                  <div
                    key={pass.id}
                    className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between opacity-75"
                  >
                    <div>
                      <h3 className="font-bold text-slate-700 line-through">
                        {pass.studentName}
                      </h3>
                      <p className="text-slate-400 text-xs font-mono">
                        ID: {pass.studentId}
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4 mt-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-800">
                  Print Past Date Report
                </h3>
                <p className="text-xs text-slate-500">
                  Select any previous date and period to generate and print
                  records.
                </p>
              </div>
            </div>
            <div className="flex items-center space-x-3 w-full sm:w-auto">
              ...
              >
                ...
                {isPrintingHistorical ? 'Loading...' : 'Print Past Report'}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="hidden print:block font-sans text-black">
        <div className="text-center border-b-2 border-slate-800 pb-4 mb-6">
          <h1 className="text-2xl font-black tracking-tight">
            SmartPass Daily Activity Report
          </h1>
          <p className="text-sm font-bold mt-1">
            Class Session:{' '}
            {isPrintingHistorical ? historicalClassName : className} | Date:{' '}
            {isPrintingHistorical
              ? reportDate
            ...
            {(isPrintingHistorical ? historicalPasses : passes).length === 0 ? (
              <tr>
                <td colSpan="6" className="py-6 text-center text-slate-500">
                  No pass activity recorded for this session.
                </td>
              </tr>
            ) : (
              (isPrintingHistorical ? historicalPasses : passes).map((pass) => (
                <tr key={pass.id}>
                  ...
                  <td className="py-2 px-2">
                    {DESTINATIONS[pass.destination]?.label || pass.destination}
                  </td>
                  <td className="py-2 px-2 capitalize font-semibold">
                    {pass.status}
                  </td>
                  <td className="py-2 px-2">
                    {new Date(pass.timestamp).toLocaleTimeString([], {
                      hour: '2-digit',
                      minute: '2-digit',
                    })}
                  </td>
                  <td className="py-2 px-2">
                    {pass.updatedAt && pass.status === 'returned'
                      ? new Date(pass.updatedAt).toLocaleTimeString([], {
                          hour: '2-digit',
                          minute: '2-digit',
                        })
                      : 'N/A'}
        ...
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl p-8 max-w-lg w-full shadow-xl relative">
            ...
            <h2 className="text-2xl font-bold text-slate-800 mb-6 flex items-center">
              <Info size={28} className="text-indigo-600 mr-2" /> About
              SmartPass Hub
            </h2>
            <div className="space-y-4 text-slate-600 mb-6 text-sm">
              <p>
                Multiple teachers can log in with their own accounts to manage
                their independent rosters and passes on a single URL.
              </p>
              <p>
                Developed by <strong>Mr. M</strong> | Contact:{' '}
                <a
                  href="mailto:ben@bymindajao.net"
            ...
            <button
              type="button"
              onClick={() => setShowHelp(false)}
              className="w-full py-3 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold rounded-xl"
            >
              Close
            </button>
          </div>
